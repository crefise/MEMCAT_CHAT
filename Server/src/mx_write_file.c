...
void mx_write_file(int sock, char* filename, int chat_id) {
    //add_file_to_CHAT(get_chat_id_from_CHATS(login_my, loogin_second), get_id_from_USERS(login_my), filename);
        ...
        char* file_name2 = sqlite3_mprintf("Server/databases/files/%s", filename);
     mx_printerr("USED IN ..."); mx_printerrln(file_name);
     mx_printerr("USED IN ..."); mx_printerrln(file_name2);
   // char *file_name = concat("Server/databases/files/%i", )
    int size = 0;
